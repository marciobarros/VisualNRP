package sobol.problems.requirements.algorithm.hillclimbing;
...
public class Visualization
{
	 ...
	 */
//	private boolean[] createRandomSolution()
//	{
//		int customerCount = project.getCustomerCount();
...
//	}
	...
	private boolean[] createRandomSolutionWithNElements(int numberOfElements)
	{
		int customerCount = project.getCustomerCount();
		boolean[] solution = new boolean[customerCount];
...
//		DecimalFormat df = new DecimalFormat("#####.##");
		...
		FileWriter outFile = new FileWriter(new File(project.getName())
				.getName().replace(".txt", "") + "_70.txt");
		PrintWriter out = new PrintWriter(outFile);

		// FileWriter outFile2 = new FileWriter(project.getName() + "_best.txt",
		// true);
...
//			List<Double> listSol = new LinkedList<Double>();
			...
			{
				boolean[] solution = createRandomSolutionWithNElements(numElemens);
				// this.bestSolution =
				// createRandomSolutionWithNElements(numElemens, random);
				...
				// out.print(df.format(getAverage(listSol)) + "\t"); //+
				// df.format(getAverage(listSol)) + "\t" +
				// df.format(getStandardDeviation(listSol)) + "\t");
		...
		out.close();
		// out2.println("R\t" +
		// MainProgram.countCustomersInSolution(bestSolution) + "\t" +
		// this.fitness);
		// out2.close();
...
//					"solutions must have the same size.");
//		}
//
//		int dist = 0;
