...
public class VisIteratedLocalSearch extends IteratedLocalSearch
{
	...
	public VisIteratedLocalSearch(PrintWriter detailsFile, Project project,
			int budget, int maxEvaluations, int numberSamplingIter,
			float intervalSize, Constructor constructor) throws Exception
	{
	...
	private int executeRandomSampling(int numberSamplingIter, Project project)
	{
		...
		for (int numElemens = 1; numElemens <= project.getCustomerCount(); numElemens++)
		{
			...
			for (int deriv = 0; deriv < numberSamplingIter; deriv++)
			{
				...
				boolean[] solution = sampConstructor
						.generateSolutionWith(numElemens);
	...
	protected NeighborhoodVisitorResult visitNeighbors(Solution solution)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			solutionAsArray[customerI] = (solutionAsArray[customerI] == true) ? false
					: true;
			int numberOfCustomers = VisualHeuristics
					.numberOfCustomersServedBySolution(solutionAsArray);

			if (numberOfCustomers >= minCustomers
					&& numberOfCustomers <= maxCustomers)
			{
	...
	{
		return canAddCustomer(satisfied)
				&& (satisfied.size() - 1) >= this.minCustomers;
	}
