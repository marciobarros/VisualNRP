...
public class MainProgram
{
	...
	private List<Project> readInstances(String[] filenames) throws Exception
	{
		...
		for (String filename : filenames)
		{
			if (filename.length() > 0)
			{
				...
				System.out.println(project.getRequirementCount() + " reqs "
						+ project.getCustomerCount() + " cust");
	...
	private void runInstance(PrintWriter out, PrintWriter details, String tipo,
			Project instance, int cycles, double budgetFactor,
			float intervalSize) throws Exception
	{
		...
		{
			VisIteratedLocalSearch hcr = new VisIteratedLocalSearch(details,
					instance, budget, 10000000, 10, intervalSize, constructor);
			// IteratedLocalSearch hcr = new IteratedLocalSearch(details,
			// instance, budget, 10000000, constructor);
			// HillClimbing hcr = new HillClimbing(details, instance, budget,
			// 10000000, constructor);
			...
			String s = tipo + "; " + instance.getName() + " #" + i + "; "
					+ executionTime + "; " + hcr.getFitness() + "; "
					+ hcr.getIterations() + "; " + hcr.getIterationBestFound()
					+ "; " + hcr.printSolution(solution);
